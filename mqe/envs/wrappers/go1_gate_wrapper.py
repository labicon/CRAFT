...
class Go1GateWrapper(EmptyWrapper):
    ...
    def step(self, action):
        ...
        reward = self.gpt_reward(base_info)
        ...
        if self.contact_punishment_scale != 0:
            ...
            self.reward_buffer["contact punishment_agent 0"] = torch.mean(collide_reward.unsqueeze(1).repeat(1, self.num_agents).type(torch.float), dim=0)[0]
            self.reward_buffer["contact punishment_agent 1"] = torch.mean(collide_reward.unsqueeze(1).repeat(1, self.num_agents).type(torch.float), dim=0)[1]
        ...
        #     self.reward_buffer["success reward_agent 0"] = torch.mean(success_reward.reshape([self.env.num_envs, self.env.num_agents]).type(torch.float), dim=0)[0]
        #     self.reward_buffer["success reward_agent 1"] = torch.mean(success_reward.reshape([self.env.num_envs, self.env.num_agents]).type(torch.float), dim=0)[1]
        ...
        if self.approach_frame_punishment_scale != 0:
            ...
            if approach_left.numel() == 0 and approach_right.numel() == 0:
                self.reward_buffer["approach frame punishment"] = torch.zeros(self.num_agents, device=self.env.device)[0]
            else:
                self.reward_buffer["approach frame punishment"] = torch.mean(approach_left + approach_right, dim=0)
        ...
        if self.lin_vel_y_punishment_scale != 0:
            ...
            self.reward_buffer["command lin_vel.y punishment_agent 0"] = torch.mean(v_y_punishment, dim=0)[0]
            self.reward_buffer["command lin_vel.y punishment_agent 1"] = torch.mean(v_y_punishment, dim=0)[1]
        ...
        if self.command_value_punishment_scale != 0:
            ...
            self.reward_buffer["command value punishment_agent 0"] = torch.mean(command_value_punishment, dim=0)[0]
            self.reward_buffer["command value punishment_agent 1"] = torch.mean(command_value_punishment, dim=0)[1]
        ...
        if self.lin_vel_x_reward_scale != 0:
            ...
            reward += v_x_reward
            self.reward_buffer["lin_vel.x reward_agent 0"] = torch.mean(v_x_reward, dim=0)[0]
            self.reward_buffer["lin_vel.x reward_agent 1"] = torch.mean(v_x_reward, dim=0)[1]
    ...
    def gpt_reward(self, base_info):
        ...
        # approach reward
        distance_to_taget = torch.norm(base_pos[:, :, :2] - self.gate_pos, p=2, dim=-1)
        ...
        success_reward[base_pos[:, :, 0] > self.gate_distance.reshape(self.env.num_envs, self.env.num_agents) + 0.1] = 5.0
        ...
        # Do not assign reward for agent 1
        reward[:, 1] = 0.0
