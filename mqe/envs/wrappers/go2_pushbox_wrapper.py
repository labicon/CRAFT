...
class Go2PushboxWrapper(EmptyWrapper):
    ...
    def gpt_reward(self, state, action):
        ...
        # 1. Reward for movement toward target
        toward_target = self._box_movement_toward_target(state, action)
        toward_target_reward = toward_target * 20
        toward_target_reward = self._check_reward_shape(toward_target_reward)
        rew_dict["box_toward_target"] = torch.mean(toward_target_reward)

        # 2. Penalty/Reward for distance between agents (Collision avoidance) - max 2.0
        agent_distance = self._agent_distance(state, action)
        collision_avoidance_reward = torch.where(agent_distance > 0.5, 2.0, 0.0)
        collision_avoidance_reward = self._check_reward_shape(collision_avoidance_reward)
        rew_dict["collision_avoidance"] = torch.mean(collision_avoidance_reward)

        # 3. Success reward for reaching the target - max 6.0
        success = self._success_evaluation(state, action)
        success_reward = success * 6.0
        success_reward = self._check_reward_shape(success_reward)
        rew_dict["success"] = torch.mean(success_reward)

        # Total Reward
        reward = (toward_target_reward + collision_avoidance_reward + success_reward)

        # Normalize and clamp the total reward
